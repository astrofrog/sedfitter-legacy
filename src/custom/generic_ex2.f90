...
program generic_ex2
  ...
  do source_id=1,number_sources(unit_in)
     ...
     write(unit,'("FIT # --------- MODEL NAME --------- --- CHI2 --- -- Av -- - Logd -",'//trim(c_n_par)//'(1X,"- ",A10," -"))') &
          & (par(p)%name,p=1,n_par)

     do f=1,n_fits
        write(unit,'(I5,1X,A30,1X,F12.2,1X,2(F8.2,1X),'//trim(c_n_par)//'(1X,2X,ES10.3,2X))') &
             & f,model_name(f),chi(f),av(f),sc(f),(par(p)%value(f),p=1,n_par)
     end do
